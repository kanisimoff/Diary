...
namespace Diary.DbStore.MongoDb.Identity
{
    ...
    public class UserStore : UserStore<IdentityUser>
    {
        ...
        /// <param name="describer">The <see cref="IMongoDbContext"/>.</param>
    ...
    public class UserStore<TUser> : UserStore<TUser, IdentityRole, IMongoDbContext, ObjectId>
    ...
    public class UserStore<TUser, TRole, TContext> : UserStore<TUser, TRole, TContext, ObjectId>
        where TUser : IdentityUser, IEntity
        where TRole : IdentityRole, IEntity
    ...
    public class UserStore<TUser, TRole, TContext, TKey> : UserStore<TUser, TRole, TContext, IdentityUserClaim, IdentityUserRole, IdentityUserLogin, IdentityUserToken, IdentityRoleClaim>
        ...
        where TRoleClaim : IdentityRoleClaim, new()
    {
        ...
        private IMongoCollection<TUser> UsersSet => Context.Set<TUser>().Result;
        private IMongoCollection<TRole> Roles => Context.Set<TRole>().Result;
        private IMongoCollection<TUserClaim> UserClaims => Context.Set<TUserClaim>().Result;
        private IMongoCollection<TUserRole> UserRoles => Context.Set<TUserRole>().Result;
        private IMongoCollection<TUserLogin> UserLogins => Context.Set<TUserLogin>().Result;
        private IMongoCollection<TUserToken> UserTokens => Context.Set<TUserToken>().Result;
        ...
        public override async Task<IdentityResult> DeleteAsync(TUser user, CancellationToken cancellationToken = default)
        {
            ...
            await UsersSet.DeleteOneAsync(u => u.Id == user.Id, cancellationToken);
            return IdentityResult.Success;
        }
        ...
        {
            var res = await Roles.FindAsync(r => r.NormalizedName == normalizedRoleName, cancellationToken:cancellationToken);
            return res.FirstOrDefault();
        }
        ...
        protected override Task<TUserRole> FindUserRoleAsync(ObjectId userId, ObjectId roleId, CancellationToken cancellationToken)
        {
            return Task.FromResult(UserRoles.AsQueryable().FirstOrDefault(ur => ur.UserId == userId && ur.RoleId == roleId));
        }
        ...
        {
            return Task.FromResult(UsersSet.AsQueryable().FirstOrDefault(u => u.Id.Equals(userId)));
        }
        ...
        {
            return Task.FromResult(UserLogins.AsQueryable().FirstOrDefault(userLogin => userLogin.UserId.Equals(userId) && userLogin.LoginProvider == loginProvider && userLogin.ProviderKey == providerKey));
        }
        ...
        {
            var res = await UserLogins.FindAsync(userLogin => userLogin.LoginProvider == loginProvider && userLogin.ProviderKey == providerKey, cancellationToken: cancellationToken);
            return res.FirstOrDefault();
        }
        ...
        public override async Task AddToRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            await UserRoles.ReplaceOneAsync(ur=>ur.Id == userRole.Id, userRole, cancellationToken: cancellationToken);
        }
        ...
        public override async Task RemoveFromRoleAsync(TUser user, string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            if (roleEntity != null)
            {
                ...
                if (userRole != null)
                {
                    await UserRoles.DeleteOneAsync(ur => ur.Id == userRole.Id, cancellationToken);
                }
            }
        }
        ...
        public override Task<IList<string>> GetRolesAsync(TUser user, CancellationToken cancellationToken = default)
        {
            ...
            var userId = user.Id;
            var rolesIds = UserRoles.AsQueryable().Where(ur => ur.UserId == userId).Select(ur=>ur.RoleId).ToList();
            return Task.FromResult<IList<string>>(Roles.AsQueryable().Where(r => rolesIds.Contains(r.Id)).Select(r=>r.Name).ToList());
        }
        ...
        public override Task<IList<Claim>> GetClaimsAsync(TUser user, CancellationToken cancellationToken = default)
        {
            ...
            return Task.FromResult<IList<Claim>>(UserClaims.AsQueryable().Where(uc => uc.UserId.Equals(user.Id)).Select(c => new Claim(c.ClaimType, c.ClaimValue)).ToList());
        }
        ...
        public override async Task RemoveClaimsAsync(TUser user, IEnumerable<Claim> claims, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var claim in claims)
            {
                await UserClaims.DeleteManyAsync(uc => uc.UserId.Equals(user.Id) && uc.ClaimValue == claim.Value && uc.ClaimType == claim.Type);
            }
        }
        ...
        public override Task<IList<UserLoginInfo>> GetLoginsAsync(TUser user, CancellationToken cancellationToken = default)
        {
            ...
            var userId = user.Id;
            var res = UserLogins.AsQueryable().Where(l => l.UserId.Equals(userId))
                .Select(l => new UserLoginInfo(l.LoginProvider, l.ProviderKey, l.ProviderDisplayName)).ToList();
            return Task.FromResult<IList<UserLoginInfo>>(res);
        }
        ...
        public override async Task<TUser> FindByEmailAsync(string normalizedEmail, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ThrowIfDisposed();
            var res = await UsersSet.FindAsync(u => u.NormalizedEmail == normalizedEmail, cancellationToken: cancellationToken);
            return res.FirstOrDefault();
        }
        ...
        public override async Task<IList<TUser>> GetUsersInRoleAsync(string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            var usersIdsInRole = UserRoles.AsQueryable().Where(ur => ur.RoleId == role.Id).Select(ur=>ur.UserId).ToList();
            ...
            CancellationToken cancellationToken)
        {
            return (await UserTokens
                .FindAsync(ut=>ut.UserId == user.Id && ut.LoginProvider == loginProvider && ut.Name == name, cancellationToken: cancellationToken))
                .FirstOrDefault();
        }
        ...
        {
            await UserTokens.ReplaceOneAsync(ut=>ut.Id == token.Id, token);
        }
        ...
        {
            await UserTokens.DeleteOneAsync(ut=>ut.Id == token.Id);
        }
    }
}
